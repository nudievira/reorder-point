...
<div class="row">
  ...
  {{-- <form class="form-horizontal"> --}}
    ...
    <div class="box-body">
      <div class="form-group">
        ...
        <div class="col-sm-6">
          <select class="form-control" name="m_supplier_id" id="m_supplier_id">
            <option value="">-Pilih-</option>
            @foreach($list_supplier as $supplier)
              <?php
              if(!empty($supp))
              {
                $select = $supp==$supplier->id?'selected':'';
              }else{
                $select = '';
              }
              ?>
              <option value="{{$supplier->id}}" {{$select}}>{{$supplier->kode}} - {{$supplier->nama}}</option>
            @endforeach
          </select>
        </div>
      </div>
  ...
  <div class="box-body">
    ...
    <div class="box-footer text-center">
      ...
      <a href="{{url('terima',$nomor)}}"  class="btn btn-info" name="print" id="print"><i class="fa fa-print"></i> Print</a>
      <a href="{{url('terima')}}" class="btn btn-danger"><i class="fa fa-remove"></i> Close</a>
    </div>
...
$('#tanggal').datepicker({
  ...
  function cariKodeBarang()
  {
    ...
    var url = "<?php echo url('terima/getDataBarang');?>";
  ...
  {
    var nomor = $("#nomor").val();
    var url = "<?php echo url('terima/getDetail');?>";
    ...
    if(x==true)
    {
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          {
            window.location.replace("{{url('terima')}}");
          }else{
            swal({
                title: "Info",
                text: data.info,
